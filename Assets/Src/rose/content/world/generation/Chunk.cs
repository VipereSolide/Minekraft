...
public class Chunk
{
            ...
            for (int x = 0; x < chunkSize.x; x++)
            {
                for (int z = 0; z < chunkSize.z; z++)
                {
                    for (int y = 0; y < chunkSize.y; y++)
                    {
                        ...
                        Tuple<BlockEntry, HashSet<Matrix4x4>> blockResult = GetRenderDataAtPosition(pos);
                        cache.Add(blockResult);
                    }
                }
            }
    ...
    protected virtual Tuple<BlockEntry, HashSet<Matrix4x4>> GetRenderDataAtPosition(Vector3Int localPosition)
    {
        ...
        HashSet<Matrix4x4> renderedFaces = new HashSet<Matrix4x4>(facesVisibleState.Length);

        for (int i = 0; i < facesVisibleState.Length; i++)
            if (facesVisibleState[i])
                renderedFaces.Add(GetRenderedFace(globalPosition, i));

        // renderedFaces.ToHashSet().RemoveWhere(x => x == null);
